...
import { Phone as PhoneIcon, Video as VideoIcon, Plus, Send, Trash2, Share, MoreVertical, Info, CheckCheck } from "lucide-react";
...
const ChatInterface = () => {
    ...
    const refreshLastSeen = () => {
        ...
        fetch(`${BACKEND_URL}/api/users/${other._id}`, { credentials: "include" })
            .then(r => r.json())
            .then(data => {
                const u = data.user;
                if (!u) return;
    ...
    useEffect(() => {
        ...
        const onMsg = (msg) => {
            ...
            if (senderId && myId && senderId.toString() !== myId.toString()) {
                fetch(`${BACKEND_URL}/api/messages/read/${conversationId}`, { method: "PUT", credentials: "include" }).catch(() => {});
            }
        };
    ...
    useEffect(() => {
        ...
        if (hasUnread) {
            fetch(`${BACKEND_URL}/api/messages/read/${conversationId}`, {
        ...
        const load = async () => {
            ...
            try {
                const [convRes, msgRes] = await Promise.all([
                    fetch(`${BACKEND_URL}/api/conversation/${conversationId}`, { credentials: "include" }),
                    fetch(`${BACKEND_URL}/api/messages/${conversationId}?limit=30`, { credentials: "include" })
                ]);
    ...
    const handleScroll = async (e) => {
        if (e.target.scrollTop < 60 && cursor && !loading) {
            ...
            try {
                const res = await fetch(`${BACKEND_URL}/api/messages/${conversationId}?limit=20&cursor=${cursor}`, { credentials: "include" });
    ...
    const deleteForMe = async (messageId) => {
        const res = await fetch(`${BACKEND_URL}/api/messages/${messageId}/me`, { method: "DELETE", credentials: "include" });
        if (res.ok) setMessages(prev => prev.filter(m => m._id !== messageId));
    };

    const deleteForEveryone = async (messageId) => {
        const res = await fetch(`${BACKEND_URL}/api/messages/${messageId}/everyone`, { method: "DELETE", credentials: "include" });
    ...
    const clearChat = async () => {
        ...
        const res = await fetch(`${BACKEND_URL}/api/messages/clear/${conversationId}`, { method: "DELETE", credentials: "include" });
    ...
    const handleVideoCall = () => {
        ...
        const other = activeChat.participants?.find(p => p._id !== (currentUser?._id || currentUser?.id));
        if (!other) return alert("Could not find participant.");
        startCall({ receiverId: other._id, receiverName: other.name, receiverPic: other.profilePic, conversationId, isVideo: true });
    };
                    ...
                    <div className="flex items-center gap-2">
                        {!activeChat?.isGroupChat && (
                            <>
                                ...
                                <button onClick={handleVideoCall} title="Start video call" className="p-2.5 rounded-xl text-gray-400 hover:text-blue-400 hover:bg-blue-500/10 transition-all active:scale-90"><VideoIcon size={19} /></button>
                            </>
                        )}
