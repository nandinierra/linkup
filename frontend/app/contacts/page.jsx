...
const ContactsPage = () => {
    ...
    useEffect(() => {
        ...
        const savedTab = localStorage.getItem(`contactsActiveTab_${currentUser._id}`);
        if (savedTab) setActiveTab(savedTab);
        else setActiveTab("contacts"); // Default to contacts
    }, [currentUser?._id]);
    ...
    const handleTabChange = (tab) => {
        setActiveTab(tab);
        if (currentUser?._id) {
            localStorage.setItem(`contactsActiveTab_${currentUser._id}`, tab);
        }
    };
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                    {activeTab === "contacts" ? (
                        <div className="max-w-4xl mx-auto">
                            ...
                            <ContactList 
                                contacts={Array.isArray(contacts) ? contacts.filter(c => c?.name?.toLowerCase().includes(searchQuery.toLowerCase())) : []} 
                                loading={loading} 
                            />
                        </div>
                    ) : activeTab === "requests" ? (
